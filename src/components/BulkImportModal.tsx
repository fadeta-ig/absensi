            ...
            <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-2xl">
                ...
                <main className="flex-1 overflow-y-auto p-6">
                    ...
                    {step === "validation" && report && <div className="space-y-5">
                        ...
                        <div><h3 className="mb-2 text-sm font-semibold">Preview rencana ({report.totalRows} baris)</h3><div className="max-h-72 overflow-auto rounded-xl border border-[var(--border)]"><Table className="text-xs"><TableHeader><TableRow><TableHead>Baris</TableHead><TableHead>Aksi</TableHead><TableHead>NIP</TableHead><TableHead>Nama</TableHead><TableHead>Departemen</TableHead><TableHead>Jabatan</TableHead><TableHead>Perubahan</TableHead></TableRow></TableHeader><TableBody>{report.rows.map((row) => <TableRow key={row.row}><TableCell>{row.row}</TableCell><TableCell><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${ACTION_STYLE[row.action]}`}>{row.action}</span></TableCell><TableCell className="font-mono">{row.employeeId}</TableCell><TableCell>{row.name}</TableCell><TableCell>{row.department}</TableCell><TableCell>{row.position}</TableCell><TableCell className="max-w-48 truncate">{row.changedFields.join(", ") || "-"}</TableCell></TableRow>)}</TableBody></Table></div></div>
                    </div>}
